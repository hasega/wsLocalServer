			...
			try {
				...
				for (FileItem item : items) {
					...
					if (item.isFormField()) {
						...
						if (item.getFieldName().equals("certificates_select"))
							certificado = item.getString();
						...
						temp.setContent( item.getInputStream()
							//	VerCert.assinadorDigital(certificado, senha, item.getInputStream(), item.getName())
								);
